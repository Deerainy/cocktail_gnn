...
功能：
1. 专门优化 balance 评分器的内部参数（μ1, μ2）
2. 使用 "原配方 > 扰动配方" 的 pairwise preference
3. 优先使用 balance 相关扰动集，如破坏风味平衡、破坏角色平衡等
...
class Config:
    ...
    # 输入输出
    OUTPUT_FILE = os.path.join(_project_root, "data", "balance_param_optimization_results.csv")
...
def generate_weight_combinations() -> List[Tuple[float, float]]:
    """
    生成权重组合
    """
    weight_combinations = []
    ...
    for i in range(Config.NUM_GRID_SAMPLES):
        # 生成均匀分布的权重
        mu1 = random.uniform(0, 1)  # μ1 (flavor balance)
        mu2 = 1 - mu1  # μ2 (role balance)
        weight_combinations.append((mu1, mu2))
    
    # Dirichlet 分布采样
    print(f"[INFO] 生成 {Config.NUM_DIRICHLET_SAMPLES} 组 Dirichlet 分布权重...")
    # 使用参数 [2, 2] 生成更集中的分布
    dirichlet_weights = dirichlet.rvs([2, 2], size=Config.NUM_DIRICHLET_SAMPLES)
    for w in dirichlet_weights:
        weight_combinations.append((w[0], w[1]))
...
def calculate_balance_score(ingredients: List[Dict], mu1: float, mu2: float) -> float:
    ...
    """
    # 临时修改全局参数
    import scripts.SQE.phase_A.sqe_scorer_balance as balance_scorer
...
def evaluate_weight_combination(weights: Tuple[float, float], 
                             recipe_id: int, 
